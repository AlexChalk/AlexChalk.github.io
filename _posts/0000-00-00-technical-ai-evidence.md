...
Several researchers have asked "What are models plausibly learning from this training approach?" and have concluded that models may be learning to misrepresent their true views or reasoning. Since model updates are limited, perhaps the limited training modifications correspond to how the model presents its views, leaving the views themselves unchanged. (one informal expression of the view is [this blog post](https://www.planned-obsolescence.org/p/the-training-game)).
...
If a training process can teach a model to do that, it seems pretty plausible that it can also teach it how to improve a machine learning model, at which point ML research has successfully been automated, and things will start to move very quickly.
...
Labs are of course motivated to claim they can create AGI for marketing and investment reasons, but I also think it's important to appreciate that AGI now appears a feasible goal to many in the research community, even if skepticism remains about how powerful a model with superhuman performance in a field like mathematics would actually be.
